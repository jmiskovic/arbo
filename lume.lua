...
function lume.deftable(proto, existing)
  existing = existing or {}
  local protoCopy
  if type(proto) == 'table' then
    protoCopy = {}
    for k, v in pairs(proto) do protoCopy[k] = v end
    setmetatable(protoCopy, getmetatable(proto))
  else
    protoCopy = proto
  end
  local mtbl = {__index = function(tbl, key) return protoCopy end}
  return setmetatable(existing, mtbl)
end

function lume.deepcopy(o, seen)
  seen = seen or {}
  ...
  if type(o) == 'table' then
    ...
    for k, v in next, o, nil do
      no[lume.deepcopy(k, seen)] = lume.deepcopy(v, seen)
    end
